...
日本語で、親切で励ましのあるアドバイスを150文字以内でお願いします。`;
               ...
               {selectedExercise ? (
                 <div>
                   ...
                   <div style={{ padding: '1rem', background: '#F0F9F7', borderRadius: '8px' }}>
                     ...
                     <button
                       onClick={() => decreaseTarget(selectedExercise.type)}
                       style={{
                         width: '100%',
                         padding: '0.5rem',
                         background: '#FFF9E6',
                 ...
                 <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
                   {availableChallenges.map(challenge => (
                     <button
                       key={challenge.type}
                       onClick={() => setSelectedExercise(challenge)}
                       style={{
                         padding: '0.75rem',
                         background: '#F0F9F7',
                     ...
                     >
                       <div style={{ fontSize: '18px', marginBottom: '0.25rem' }}>{challenge.icon}</div>
                       <div style={{ fontWeight: 500, color: '#0F6E56' }}>{challenge.name}</div>
                 ...
                 <div style={{ maxHeight: '200px', overflowY: 'auto', fontSize: '12px' }}>
                   {messages.map(msg => (
                     <div
                       key={msg.id}
                       style={{
                         background: msg.from === 'system' ? '#FFF9E6' : '#E3F2FD',
                         padding: '0.75rem',
                         borderRadius: '6px',
                         marginBottom: '0.5rem',
                         lineHeight: 1.5,
     ...
     // 夫さん用ビュー
     return (
       <div style={{ background: 'linear-gradient(135deg, #FFEBEE 0%, #FFF 100%)', minHeight: '100vh', padding: '1rem' }}>
         <div style={{ maxWidth: '500px', margin: '0 auto', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
           <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
             <h1 style={{ fontSize: '22px', fontWeight: 500, margin: 0, color: '#C62828' }}>
               💪 妻の運動管理画面
             </h1>
             <button
           ...
           >
             ...
             <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
               <div style={{ background: '#FFEBEE', padding: '0.75rem', borderRadius: '8px', textAlign: 'center' }}>
                 <div style={{ fontSize: '18px', marginBottom: '0.25rem' }}>🏆</div>
                 <div style={{ fontWeight: 500, color: '#C62828' }}>{completedCount}</div>
                 <div style={{ fontSize: '11px', color: '#999' }}>チャレンジ達成</div>
               </div>
               <div style={{ background: '#FFEBEE', padding: '0.75rem', borderRadius: '8px', textAlign: 'center' }}>
                 <div style={{ fontSize: '18px', marginBottom: '0.25rem' }}>⭐</div>
                 <div style={{ fontWeight: 500, color: '#C62828' }}>レベル {level}</div>
                 <div style={{ fontSize: '11px', color: '#999' }}>現在のレベル</div>
               </div>
             </div>
           </div>
                 ...
                 {challenges.map((c, idx) => (
                   ...
                   >
                     <div style={{ fontWeight: 500 }}>
                       {exerciseProgression[c.exerciseType]?.name || 'Unknown'}{' '}
                       {exerciseProgression[c.exerciseType]?.icon}
                     </div>
                     <div style={{ fontSize: '11px', color: '#666' }}>
                       {c.reps}
                       {exerciseProgression[c.exerciseType]?.unit || '回'} / 目標：{c.targetWas}
                       {exerciseProgression[c.exerciseType]?.unit || '回'}
                     </div>
                   </div>
                 ))}
               </div>
